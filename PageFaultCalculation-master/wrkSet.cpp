/* Page Fault Calculation Using Working Set Algorithm */
...
int LIFO()
{
 ...
 cout << "You have compiled only Working Set !!!" << endl;
 return 0;
}
...
int wrkSet()
{
	...
	fileOP << "*******************************************************" << endl;
	fileOP << "Page Fault Calculation - Working Set Algorithm Used" << endl;
	fileOP << "*******************************************************" << endl;
 	while(!fileIP.eof())
	{
					    ...
					    if ( DT1->logAddress[2] > 0 )
							{
								...
								if( (pQueue[iBuff].page.size() < TotalNumberPageFramesPerProcess) )
							    {
								    ...
								    *DT2 = tempDiskReq;
								    ...
								    for ( int jBuff = 0; jBuff < pQueue[iBuff].page.size(); jBuff++ )
									{	int tmpCount = 0;
									    for ( int kBuff = 0; kBuff < jBuff +1; kBuff++ )
										{	if ( pQueue[iBuff].page[jBuff] == pQueue[iBuff].page[kBuff] )
												tmpCount++;
										}
										if ( tmpCount == 1 )
												fileOP << pQueue[iBuff].page[jBuff] << " "; 
									}
							  ...
							  else
							  {
								  ...
								  do
								  {
									  ...
									  if(pQueue[iBuff].page.at(jBuff) == DT1->logAddress[2])
									  {
									    deque <char> bufferQ;
									    do{
										   if(pQueue[iBuff].page.back() != DT1->logAddress[2])
											bufferQ.push_front(pQueue[iBuff].page.back());	
										   pQueue[iBuff].page.pop_back();
										   }while(!pQueue[iBuff].page.empty());
								  ...
								  if(!bFound)
								  {
									  ...
									  do
									  {
										  if(vecFrames[jBuff].pid == DT1->pid && vecFrames[jBuff].pagenum == pQueue[iBuff].page.back())
										  {
